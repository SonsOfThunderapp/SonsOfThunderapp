...
## Thunder Wake + Concierge 2.0 (locked 2026-08-16)

1. Thunder AI activation has a protected signature wake (`ThunderFX.thunderWake`).
2. Wake is semantic: full once per session, soft on reopen, none on follow-up sends.
3. Physical haptics only where `navigator.vibrate` exists; iPhone never pretends to vibrate.
4. Concierge teaches benefit + that brothers can Ask Thunder; demo uses real pipeline only.
5. Tour is versioned (v2), skippable, replayable — no spam for completed tours.
6. MASTERPIECE1b visual DNA must not regress for wake/tour work.

## Security (sec1)

See SECURITY.md. Client is untrusted. LEADER_PIN is UI-only. Push list is service-role only. Brothers writes require owner_id = auth.uid().
