...
$('#thunder-input').addEventListener('keydown', (e) => {
  ...
  function maybeStartProductTour() {
    /* LOCKED surface 2026-08-16: do NOT auto-start Follow the Bolt.
       Multi-step coach marks on Home felt corporate and covered I’M IN.
       Tour remains available only via More → replay control. */
    try {
      if (!isTourComplete()) {
        setTourState({ complete: true, skipped: true, step: 'auto-suppressed', suppressedAt: Date.now() });
      }
    } catch (e) {}
  }
