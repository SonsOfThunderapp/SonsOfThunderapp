...
(function () {
  ...
  /* ── HERO EFFECTS (scarce) ─────────────────────────────────────
     ...
     APP LASER IGNITION — only on cold splash (session not yet done)
  ...
  function runThunderLaserIgnition() {
    try {
      if (document.getElementById('tb-laser')) return;
      const reduced = prefersReducedMotionHero();
      const layer = document.createElement('div');
      layer.id = 'tb-laser';
      layer.className = 'tb-hero-layer tb-laser-layer';
      layer.setAttribute('aria-hidden', 'true');
      if (reduced) {
        layer.classList.add('tb-hero-reduced', 'tb-laser-reduced');
        document.body.appendChild(layer);
        setTimeout(() => { try { layer.remove(); } catch (e) {} }, 500);
        return;
      }
      layer.innerHTML = [
        '<div class="tb-laser-streak tb-laser-a"></div>',
        '<div class="tb-laser-streak tb-laser-b"></div>',
        '<div class="tb-laser-streak tb-laser-c"></div>',
        '<div class="tb-laser-streak tb-laser-d"></div>',
        '<div class="tb-laser-core"></div>'
      ].join('');
      document.body.appendChild(layer);
      // Android tactile: short-pause-short-stronger (only if API exists)
      try {
      ...
      setTimeout(() => {
        try { layer.classList.add('tb-laser-done'); } catch (e) {}
      }, 720);
      setTimeout(() => {
        try { layer.remove(); } catch (e) {}
      }, 1050);
  ...
  function thunderActionsForQuery(query, localResult) {
    ...
    if (q.includes('next meeting') || q.includes('when is') || q.includes('next gathering') || q.includes('what time') || q.includes('where')) {
      return [{ id: 'calendar', label: 'ADD TO CALENDAR' }];
    }
    if (q.includes("who's in") || q.includes('rsvp') || q.includes("i'm in") || q.includes('im in')) {
      return rsvp ? null : [{ id: 'im_in', label: "I'M IN" }];
    }
    if (q.includes('rough night') || q.includes('struggling') || q.includes('need to talk') || q.includes('who can i call') || q.includes('alone') || q.includes('depressed') || q.includes('anxious')) {
      return [{ id: 'text_leader', label: 'TEXT A LEADER' }];
    }
    if (q.includes('who can help') || q.includes('brother') || q.includes('directory')) {
      return [{ id: 'brothers', label: 'VIEW BROTHERS' }];
    }
    return null;
  }
...
$('#edit-profile-btn').addEventListener('click', () => {
    ...
    $('#thunder-fab').addEventListener('click', () => {
      try { tbFeedback.press($('#thunder-fab')); } catch (e) {}
...
$('#thunder-input').addEventListener('keydown', (e) => {
  ...
  function runSplash() {
    ...
    // HERO: Laser ignition on genuine cold splash only (session gate above)
    try {
    ...
    // Hold for bolt settle + laser settle, then dramatic zoom → welcome
  ...
  // ---------- PRODUCT TOUR (versioned first-run coach marks) ----------
  const TB_TOUR_VERSION = 1;
  ...
  const TB_TOUR_STEPS = [
    {
      id: 'home',
      view: 'home',
      target: '#view-home .next-meeting',
      headline: 'THIS IS HOME',
      body: 'Everything that matters right now starts here.',
      nextLabel: 'NEXT'
    },
    {
      id: 'gathering',
      view: 'home',
      target: '#meeting-date',
      headline: 'KNOW WHEN WE MEET',
      body: 'Date, time, place — always current.',
      nextLabel: 'NEXT'
    },
    {
      id: 'imin',
      ...
      headline: 'SHOW UP',
      body: 'Tap I’M IN when you’re locking in your seat.',
      nextLabel: 'NEXT',
      allowTargetTap: true
    },
    {
      ...
      headline: 'KNOW YOUR BROTHERS',
      body: 'Profiles help us know who’s in the room.',
      nextLabel: 'NEXT'
    },
    {
      id: 'events',
      ...
      headline: 'KEEP THE NIGHTS',
      body: 'Events and memories live here.',
      nextLabel: 'NEXT'
    },
    {
      id: 'thunder',
      ...
      body: 'Gathering, The Code, Scripture — ask instead of hunting.',
      nextLabel: 'NEXT'
    },
    {
      id: 'code',
      view: 'about',
      target: '.code-title',
      headline: 'REMEMBER WHY',
      body: 'The Code and who we are — always with you.',
      nextLabel: 'NEXT'
    },
    {
      id: 'alerts',
      view: 'about',
      target: '#gathering-alerts-card',
      headline: 'DON’T MISS THE CALL',
      body: 'Gathering Alerts for announcements. You choose when.',
      nextLabel: 'FINISH'
    }
  ];
  ...
  function placeTourTip(target) {
    ...
    if (!tip || !spot) return;
    const pad = 10;
    const rect = target ? target.getBoundingClientRect() : { top: 80, left: 24, width: window.innerWidth - 48, height: 40, bottom: 120 };
    ...
    let tipTop;
    if (spaceBelow > tipH + 24 || spaceBelow >= spaceAbove) {
      tipTop = Math.min(window.innerHeight - tipH - 12, rect.bottom + 14);
    } else {
      tipTop = Math.max(12, rect.top - tipH - 14);
    }
    // Keep clear of bottom nav
    const maxTop = window.innerHeight - tipH - 64;
    tipTop = Math.max(12, Math.min(tipTop, maxTop));
  ...
  function renderTourStep() {
    ...
    // Wait a tick for view paint
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (__tourTargetEl) {
          try { __tourTargetEl.classList.remove('tb-tour-target-pulse'); } catch (e) {}
        }
        ...
        if (el) {
          ...
          try { el.classList.add('tb-tour-target-pulse'); } catch (e) {}
        }
        placeTourTip(el);
      });
    });
  ...
  function maybeStartProductTour() {
    ...
    // After welcome / splash settle
    setTimeout(() => {
      try {
        if (isTourComplete()) return;
        if (document.getElementById('welcome') && !document.getElementById('welcome').classList.contains('hidden')) return;
        startTour();
      } catch (e) {}
    }, 900);
  }
  ...
  function bindTourControls() {
    ...
    if (dim && !dim.dataset.tbBound) {
      ...
      // dim tap does not skip — avoid accidents; only SKIP / NEXT
    }
  ...
  function runWelcome() {
    ...
    const dismiss = () => {
      ...
      el.addEventListener('transitionend', hide, { once: true });
      setTimeout(hide, 600); // match softer exit
    };
    ...
    // 7s or tap — product rule unchanged
  ...
  async function init() {
    ...
    try {
      ...
      if (sessionStorage.getItem('tb_welcome_done') === '1') {
        setTimeout(() => { try { maybeStartProductTour(); } catch (e) {} }, 1200);
      }
    } catch (e) {}
    bootstrapSeenState();
    updateMeetingCard();
