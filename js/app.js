...
(function () {
  ...
  async function pullBrothers() {
    ...
    try {
      ...
      // Merge: prefer remote roster; keep local-only rows not yet pushed
...
$('#thunder-input').addEventListener('keydown', (e) => {
    ...
    if (annAdd) {
      annAdd.addEventListener('click', async () => {
        ...
        // Publish for all brothers when signed in
        if (supabaseEnabled()) {
          if (!isSignedIn()) {
            alert('Saved on this phone.\nSign in (Events → account) then add again — or use Leadership Save path after sign-in — to publish for every brother.');
          } else {
            const res = await pushAnnouncements();
            if (res && res.ok) {
              /* quiet success */
            } else {
              alert('Saved on this phone. Shared publish failed' + (res && res.reason ? ' (' + res.reason + ')' : '') + '.');
            }
          }
        }
      });
  ...
  function runSplash() {
    ...
    const finish = () => {
      ...
      setTimeout(hide, 1300); // fail-open if transitionend misses (matches slower zoom)
    };

    // Hold ~3.6s then slow zoom into viewer + fade (total ~4.7s)
