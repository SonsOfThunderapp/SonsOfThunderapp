...
(function () {
  ...
  let announcementsSeenAt = load('announcementsSeenAt');
  if (typeof announcementsSeenAt !== 'number') announcementsSeenAt = 0;
  ...
  async function sbUpload(bucket, path, dataUrlOrBlob, contentType) {
    const base = cfg().SUPABASE_URL.replace(/\/$/, '');
    const key = cfg().SUPABASE_ANON_KEY;
    ...
    if (typeof dataUrlOrBlob === 'string' && dataUrlOrBlob.startsWith('data:')) {
      body = dataUrlToBlob(dataUrlOrBlob);
      type = body.type || type;
    }
  ...
  async function pullBrothers() {
    ...
    const rows = await sbFetch('/rest/v1/brothers?select=*&order=name.asc', {
      method: 'GET',
      headers: sbHeaders()
    });
    ...
    save('brothers', brothers); // local cache for offline
    return true;
  }
  ...
  async function pushBrother(entry) {
    ...
    // Upload data-URL photos to Storage
    if (photoUrl && String(photoUrl).startsWith('data:')) {
      const ext = (photoUrl.split(';')[0].split('/')[1] || 'jpg').replace('jpeg', 'jpg');
      const path = entry.id + '/avatar.' + ext;
      photoUrl = await sbUpload('brother-photos', path, photoUrl, null);
    }
    const row = {
      id: entry.id,
      name: entry.name,
    ...
    await sbFetch('/rest/v1/brothers?on_conflict=id', {
      method: 'POST',
      headers: sbHeaders({ 'Prefer': 'resolution=merge-duplicates,return=representation' }),
      body: JSON.stringify(row)
    });
    return Object.assign({}, entry, { photo: photoUrl });
  }

  async function pullMemories() {
    if (!supabaseEnabled()) return false;
    const rows = await sbFetch('/rest/v1/memories?select=*&order=created_at.desc&limit=60', {
      method: 'GET',
      headers: sbHeaders()
    });
    if (!Array.isArray(rows)) return false;
  ...
  async function pushMemory(item) {
    if (!supabaseEnabled()) return item;
    let url = item.data;
    if (url && String(url).startsWith('data:')) {
      const isVideo = item.type === 'video';
      const ext = isVideo ? 'mp4' : 'jpg';
      const path = (item.brother_id || 'anon') + '/' + Date.now() + '.' + ext;
      url = await sbUpload('memories', path, url, isVideo ? 'video/mp4' : 'image/jpeg');
    }
    const row = {
      brother_id: item.brother_id || myProfileId || null,
      caption: item.caption || '',
      media_url: url,
      media_type: item.type === 'video' ? 'video' : 'image'
    };
    const saved = await sbFetch('/rest/v1/memories', {
      method: 'POST',
    ...
    return {
      ...
      type: item.type,
      caption: item.caption || '',
      date: (rec && rec.created_at) || item.date,
  ...
  async function syncSharedData() {
    ...
    try {
      ...
      console.warn('Shared sync failed (using local cache)', e);
    }
  }
  ...
  // ---------- RENDER ----------
  function latestAnnouncementAt() {
    if (!announcements.length) return 0;
    return Math.max(...announcements.map(a => a.createdAt || 0));
  }

  function hasNewAnnouncements() {
    const latest = latestAnnouncementAt();
    // Ignore legacy defaults with createdAt 0 so first load isn't always "NEW"
    if (!latest) return false;
    return latest > (announcementsSeenAt || 0);
  }

  function markAnnouncementsSeen() {
    const latest = latestAnnouncementAt();
    if (latest && latest > (announcementsSeenAt || 0)) {
      announcementsSeenAt = latest;
      save('announcementsSeenAt', announcementsSeenAt);
    }
    renderAnnouncements();
  ...
  function updateAnnouncementsBadge() {
    ...
    if (!title) return;
    const show = hasNewAnnouncements();
    let badge = title.querySelector('.new-badge');
    if (show) {
      if (!badge) {
  ...
  function renderAnnouncements() {
    ...
    el.innerHTML = announcements.map(a => {
      const isNew = (a.createdAt || 0) > (announcementsSeenAt || 0) && (a.createdAt || 0) > 0;
      return `
      <div class="announcement-card${isNew ? ' announcement-new' : ''}">
  ...
  function bindBrotherDetail() {
    ...
    if (closeBtn) closeBtn.addEventListener('click', (e) => { e.stopPropagation(); closeBrotherDetail(); });
  ...
  function renderBrothers() {
    ...
    grid.innerHTML = brothers.map((b, i) => {
      ...
      return `
        <button type="button" class="brother-card" data-brother-index="${i}" aria-label="View ${esc(b.name || 'brother')} profile">
          ${photoHtml}
  ...
  function bindMemoryViewer() {
    ...
    if (closeBtn) closeBtn.addEventListener('click', (e) => { e.stopPropagation(); closeMemoryViewer(); });
    // Tap dark chrome (not the image) to close — stage itself doesn't close so video controls work
    viewer.addEventListener('click', (e) => {
      if (e.target === viewer || e.target.classList.contains('memory-viewer-top')) {
        closeMemoryViewer();
      }
    });
    ...
    viewer.addEventListener('touchend', (e) => {
      ...
      if (Math.abs(dx) < 50 || Math.abs(dx) < Math.abs(dy)) return; // need horizontal swipe
      if (dx < 0) memoryViewerStep(1);      // swipe left → next
      else memoryViewerStep(-1);           // swipe right → prev
    }, { passive: true });
    document.addEventListener('keydown', (e) => {
  ...
  function renderMedia() {
    ...
    el.innerHTML = media.map((m, i) => {
      ...
      const mediaTag = isVideo
        ? `<video src="${m.data}" muted playsinline preload="metadata"></video><div class="media-thumb-play">▶</div>`
        : `<img src="${m.data}" alt="" loading="lazy" />`;
      const cap = m.caption ? `<div class="media-thumb-cap">${esc(m.caption)}</div>` : '';
      return `<button type="button" class="media-thumb" data-media-index="${i}" aria-label="View memory">${mediaTag}${cap}</button>`;
    }).join('');
    el.querySelectorAll('.media-thumb').forEach(btn => {
      btn.addEventListener('click', () => {
  ...
  function showView(name) {
    ...
    if (name === 'home') {
      // Let NEW show briefly, then mark read so it clears on this visit
      clearTimeout(showView._annTimer);
      showView._annTimer = setTimeout(() => markAnnouncementsSeen(), 2800);
    }
  }
  ...
  function bindEvents() {
    ...
    $('#save-profile').addEventListener('click', async () => {
      ...
      try {
        ...
        pendingPhotoData = null;
        if (supabaseEnabled()) {
          // brief refresh so everyone sees latest order
          try { await pullBrothers(); renderBrothers(); } catch (e) {}
        }
      } catch (err) {
        console.error(err);
        alert('Could not save profile to shared roster. Saved on this phone only.\n' + (err.message || ''));
        // still save locally
        if (existing >= 0) brothers[existing] = entry;
        else brothers.unshift(entry);
        save('brothers', brothers);
      ...
      reader.onload = (ev) => {
        ...
        const finish = async (dataUrl) => {
          const item = {
            ...
            brother_id: myProfileId || null
          };
          try {
            let saved = item;
            if (supabaseEnabled()) {
              saved = await pushMemory(item);
            }
            media.unshift(saved);
            try { save('media', media); } catch (e) {
              // if local full but remote ok, keep remote list slim in memory
              media = media.slice(0, 20);
              try { save('media', media); } catch (e2) {}
            }
            renderMedia();
            renderLastFire();
            ...
            console.error(err);
            alert('Could not save memory. ' + (err.message || 'Try a smaller photo.'));
          }
        };
