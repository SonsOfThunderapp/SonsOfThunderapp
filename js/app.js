  ...
  function bindEvents() {
    ...
    function refreshInstallCta() {
      ...
      if (isInAppBrowser()) {
        ...
        if (tip) tip.textContent = 'Instagram / Messenger → Open in Safari, then install';
      } else if (deferredInstallPrompt) {
        if (installShareBtn) installShareBtn.textContent = 'INSTALL THUNDER BOARD';
        if (title) title.textContent = 'PUT THUNDER BOARD ON YOUR HOME SCREEN';
        if (sub) sub.textContent = 'One confirm · native Android install';
        ...
        if (title) title.textContent = 'PUT THUNDER BOARD ON YOUR HOME SCREEN';
        if (sub) sub.textContent = '3 quick taps in Safari';
        if (tip) tip.textContent = 'Share → Add to Home Screen → Add';
      } else {
        if (installShareBtn) installShareBtn.textContent = 'INSTALL / SHARE';
        if (title) title.textContent = 'PUT THUNDER BOARD ON YOUR HOME SCREEN';
