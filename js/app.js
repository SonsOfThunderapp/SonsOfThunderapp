...
$('#thunder-input').addEventListener('keydown', (e) => {
  ...
  window.ThunderCharacter = (function () {
    ...
    function img() {
      return document.getElementById('thunder-fab-img');
    }
    ...
    function flashSmile(ms) {
      ...
      try {
        if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
          return; // keep cool face; no flash
        }
      } catch (e) {}
      if (smileTimer) {
      ...
      if (fab) {
        fab.classList.add('fab-hit');
        setTimeout(() => { try { fab.classList.remove('fab-hit'); } catch (e) {} }, 480);
      }
    ...
    return { init: init, applyCool: applyCool, flashSmile: flashSmile };
  })();
