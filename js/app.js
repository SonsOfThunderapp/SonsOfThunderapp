...
(function () {
  ...
  function hasNewLastFire() {
    return false; // Home Last Fire disabled — memories stay on Events
  }
  ...
  function renderLastFire() {
    // Memories live only on Events (thumbnail grid). Do not show on Home.
    const el = $('#last-fire');
    if (!el) return;
    el.classList.add('hidden');
    el.innerHTML = '';
  ...
  function thunderRespondLocal(q) {
    ...
    if (query.includes("who's in") || query.includes('who is going') || query.includes('rsvp') || query.includes("i'm in") || query.includes('im in')) {
      return {
        text: "Tap **I'M IN** on Home to lock in for the next gathering.",
  ...
  function bindEvents() {
    ...
    function refreshInstallCta() {
      ...
      // Already on home screen → hide the whole CTA
      if (isStandalonePwa()) {
      ...
      if (deferredInstallPrompt) {
        if (installShareBtn) installShareBtn.textContent = 'INSTALL APP';
        if (title) title.textContent = 'PUT THUNDER BOARD ON YOUR HOME SCREEN';
        if (sub) sub.textContent = 'One tap · native Android install';
        if (tip) tip.textContent = 'Opens your phone’s install prompt';
      } else if (isIos()) {
        if (installShareBtn) installShareBtn.textContent = 'SHARE TO HOME SCREEN';
        if (title) title.textContent = 'PUT THUNDER BOARD ON YOUR HOME SCREEN';
        if (sub) sub.textContent = 'iPhone: Share → Add to Home Screen';
        if (tip) tip.textContent = 'Safari → Share → Add to Home Screen';
      } else {
        if (installShareBtn) installShareBtn.textContent = 'SHARE / COPY LINK';
        if (title) title.textContent = 'PUT THUNDER BOARD ON YOUR HOME SCREEN';
        if (sub) sub.textContent = 'Chrome menu → Install app / Add to Home screen';
        if (tip) tip.textContent = 'Or tap HOW for the short walkthrough';
      }
    }
    ...
    async function runSmartInstall() {
      ...
      // iPhone / fallback: native Share sheet (often includes Add to Home Screen)
      ...
      if (navigator.share) {
        try {
          ...
          if (isIos()) showInstallToast('In Share, tap Add to Home Screen');
          else showInstallToast('Shared. Or use Chrome menu → Install app');
          return;
        } catch (err) {
      ...
      try {
        ...
        showInstallToast('Link copied. Safari/Chrome → Share or menu → Add to Home Screen');
      } catch (e) {
        showInstallToast('Open in Safari/Chrome → menu → Add to Home Screen');
      }
    }
...
$('#thunder-input').addEventListener('keydown', (e) => {
  ...
  function runSplash() {
    ...
    // Hold ~3s then zoom/fade out (total ~3.5–3.6s) — mark glow runs during hold
