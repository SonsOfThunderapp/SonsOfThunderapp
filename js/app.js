...
(function () {
  ...
  async function pushEventsBoard() {
    ...
    // Leadership PIN is the gate in the UI — do not require memories sign-in to publish
    const sb = getSb();
  ...
  async function pushAnnouncements() {
    ...
    const sb = getSb();
    if (!sb) return { ok: false, reason: 'no_client' };
    // Leadership PIN gates the UI — no memories sign-in required to publish
    try {
  ...
  async function pushBrother(entry) {
    ...
    try {
      ...
      if (error) {
        ...
        // still return local entry — phone save already done by caller
        return entry;
      }
  ...
  function elasticClear(el) {
    ...
    el.style.transition = '';
    el.style.transform = '';
    el.classList.remove('elastic-dragging', 'elastic-settling');
  }
  ...
  function bindElasticSwipe(root, opts) {
    ...
    root.addEventListener('touchmove', (e) => {
      ...
      if (axis !== 'x' || prefersReducedMotion()) return;
      // live drag with resistance
      let tx = dx;
  ...
  // Cash App–style QR: client-side (qrcode.min.js) + official bolt center overlay
  // ECC H so scanners still read with the logo over the quiet center.
  ...
  function renderBrotherQR(vcard, targetEl, size) {
    ...
    if (typeof QRCode === 'undefined') {
      // Fallback: remote image (bolt pad still overlays via CSS parent .qr-stage)
      const img = document.createElement('img');
      img.width = dim;
      img.height = dim;
      img.alt = 'Contact QR';
      img.src = 'https://api.qrserver.com/v1/create-qr-code/?size=' + dim + 'x' + dim + '&ecc=H&margin=8&data=' + encodeURIComponent(vcard);
  ...
  function showContactQR(brother) {
    ...
    const vcard = buildVCard(brother);
  ...
  function showView(name, opts) {
    ...
    $$('.view').forEach(v => v.classList.remove('active', 'view-swipe-in'));
    ...
    if (view) {
      ...
      if (fromSwipe) {
        ...
        // one-shot; remove after paint so next show is clean
    ...
    $('#rsvp-btn').addEventListener('click', () => {
      ...
      if (btn && rsvp) {
        if (!prefersReducedMotion()) {
          ...
          setTimeout(() => btn.classList.remove('lock-pulse'), 500);
        }
      ...
      try {
        ...
        thunderRecognition.onerror = (ev) => {
          ...
          if (ev && ev.error === 'not-allowed') {
            const hint = $('#thunder-voice-hint');
            if (hint) hint.textContent = 'Mic blocked — allow microphone or type below';
          }
        };
...
$('#thunder-input').addEventListener('keydown', (e) => {
  ...
  async function loadActivityFeed(key) {
    ...
    // Toggle off if same tag tapped again
    ...
    if (activityFeedCache[key] && (Date.now() - activityFeedCache[key].at < 10 * 60 * 1000)) {
      el.innerHTML = renderActivityItems(activityFeedCache[key].items, conf.source);
      return;
    }
    ...
    try {
      ...
      // Only paint if still the active tag
      if (activeActivityFeed === key) {
        el.innerHTML = renderActivityItems(items, conf.source);
      }
    } catch (e) {
      console.warn('activity feed', key, e);
      if (activeActivityFeed === key) {
        el.innerHTML = '<div class="empty-state">Couldn’t load ' + esc(conf.label) + ' feed. Try again later.</div>';
      }
    }
  }
  ...
  function selectActivityFeed(key) {
    // Always select (no toggle-off) — used by swipe + first open
    if (!ACTIVITY_FEEDS[key]) return;
    if (activeActivityFeed === key) {
      // re-paint from cache if needed
      const el = $('#activity-feed');
      const conf = ACTIVITY_FEEDS[key];
    ...
    // force load even if same was toggled — bypass toggle-off by clearing first
    const prev = activeActivityFeed;
    activeActivityFeed = null;
  ...
  function bindActivityTags() {
    ...
    // Elastic swipe on feed chips area: RANGE → LAKE → BIBLE → GYM
    const swipeRoot = wrap.parentElement || wrap;
    if (swipeRoot.dataset.feedSwipe === '1') return;
    swipeRoot.dataset.feedSwipe = '1';
    const feedEl = $('#activity-feed') || wrap;
    bindElasticSwipe(swipeRoot, {
      getEl: () => feedEl.classList.contains('hidden') ? wrap : feedEl,
      canPrev: () => {
      ...
      canNext: () => {
        let i = FEED_ORDER.indexOf(activeActivityFeed);
        if (i < 0) i = 0;
        return i < FEED_ORDER.length - 1;
      },
      onPrev: () => {
        let i = FEED_ORDER.indexOf(activeActivityFeed);
        if (i < 0) i = 0;
        if (i > 0) selectActivityFeed(FEED_ORDER[i - 1]);
      },
      onNext: () => {
        let i = FEED_ORDER.indexOf(activeActivityFeed);
        if (i < 0) i = 0;
        if (i < FEED_ORDER.length - 1) selectActivityFeed(FEED_ORDER[i + 1]);
      },
      blocked: (t) => !!(t && t.closest && (
        t.closest('#media-feed') || t.closest('#upcoming-events') ||
        t.closest('#next-mission-card') || t.closest('#upload-media-btn') ||
        t.closest('a.activity-card')
      ))
    });
  }
