...
(function () {
  ...
  function setupTabSwipe() {
    ...
    let tracking = false;
    const THRESH = 72;
    const RATIO = 1.25; // horizontal must dominate vertical
    ...
    root.addEventListener('touchend', (e) => {
      ...
      if (absX < absY * RATIO) return; // vertical scroll / down-swipe wins

      const idx = TAB_ORDER.indexOf(currentViewName);
      if (idx < 0) return;
      // swipe left → next; swipe right → previous; no loop
