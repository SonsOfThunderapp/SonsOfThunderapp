...
(function () {
  ...
  function addThunderMsg(text, role, source) {
    ...
    div.innerHTML = text + (source ? `<div class="source">Source: <span>${source}</span></div>` : '');
    box.appendChild(div);
  ...
  async function thunderRespondGrok(q) {
    ...
    const res = await fetch('/.netlify/functions/thunder-ai', {
      method: 'POST',
  ...
  async function handleThunderSend() {
    ...
    try {
      ...
      if (pending) pending.remove();
      addThunderMsg(
        `Can’t reach Thunder right now. Try again in a minute, or <strong>Text a Leader</strong> from Home.`,
        'assistant',
        null
      );
  ...
  function runSplash() {
    ...
    // Hold ~1s then zoom/fade out (total ~1.5s)
