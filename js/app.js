...
(function () {
  ...
  function renderRsvp() {
    ...
    if (rsvp) {
      ...
      if (status) {
        status.textContent = "⚡ YOU'RE LOCKED IN · this phone";
        status.classList.remove('hidden');
      }
  ...
  function bindEvents() {
    ...
    $('#rsvp-btn').addEventListener('click', () => {
      ...
      setTimeout(() => { if (btn) btn.dataset.busy = '0'; }, 450);
      const wasIn = !!rsvp;
      ...
      if (!ok) {
        ...
        alert("Couldn’t lock that in on this phone (storage full). Free a little space and try again.");
      ...
      authSignUpBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (data && data.session) {
            ...
            setAuthError('If invite-only is on, ask a leader to add you. Otherwise check email to confirm, then sign in.');
          }
        } catch (e) {
        ...
        if (supabaseEnabled()) {
          ...
          try {
            ...
            if (res && res.ok) {
              ...
              alert('Saved on this phone. Sign in (Events → account) so shared publish can reach every brother.');
            } else {
