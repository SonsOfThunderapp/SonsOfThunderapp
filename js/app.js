...
(function () {
  ...
  function clearQrTarget() {
    const target = $('#qr-code-target');
    if (target) target.innerHTML = '';
    qrcodeInstance = null;
  }

  function renderBrotherQR(vcard) {
    const target = $('#qr-code-target');
    if (!target) return false;
    clearQrTarget();
    if (typeof QRCode === 'undefined') {
      // Fallback: remote image only (no bolt composite)
      const img = document.createElement('img');
      img.width = 260;
      img.height = 260;
      img.alt = 'Contact QR';
      img.src = 'https://api.qrserver.com/v1/create-qr-code/?size=260x260&ecc=H&margin=10&data=' + encodeURIComponent(vcard);
  ...
  function updateMyQrButtonVisibility() {
    ...
    if (!btn) return;
    const me = (brothers || []).find(b => b.id === myProfileId);
    const has = !!(me && digitsOnly(me.phone) && me.name);
  ...
  function openBrotherDetail(index) {
    ...
    // Contact actions — phone is opt-in only
    const actions = $('#brother-contact-actions');
    const callA = $('#brother-call');
    const textA = $('#brother-text');
    const shareBtn = $('#brother-share-contact');
    const qrBtn = $('#brother-show-qr');
    ...
    if (actions) {
      if (phone) {
        ...
        if (callA) {
          ...
          callA.classList.remove('hidden');
        }
        if (textA) {
          textA.href = 'sms:' + phone;
        ...
        if (qrBtn) {
          qrBtn.onclick = () => showContactQR(b);
        }
      } else {
        actions.classList.add('hidden');
        if (callA) callA.classList.add('hidden');
        if (textA) textA.classList.add('hidden');
      }
    }
...
$('#thunder-input').addEventListener('keydown', (e) => {
  ...
  function runSplash() {
    ...
    const finish = () => {
      ...
      // Cross-fade: welcome starts while splash is still dissolving
      setTimeout(() => {
      ...
      setTimeout(hide, 1200); // fail-open
    };

    // Hold ~2.4s (bolt settles), then exit — less dead time on the logo
