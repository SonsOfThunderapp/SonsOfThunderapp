...
(function () {
  ...
  function renderWhosIn(list) {
    ...
    const label = n === 1 ? '1 BROTHER IS IN' : (n + ' BROTHERS ARE IN');
    ...
    row.innerHTML = shown.map(p => {
      ...
      if (p.photo) {
        return `<div class="whos-chip${isMe ? ' me' : ''}" title="${esc(p.initials || '')}"><img src="${esc(p.photo)}" alt=""></div>`;
      }
      return `<div class="whos-chip${isMe ? ' me' : ''}">${esc(p.initials || '?')}</div>`;
  ...
  function bindEvents() {
    ...
    function refreshInstallCta() {
      ...
      if (isInAppBrowser()) {
        ...
        if (title) title.textContent = 'HOME SCREEN';
