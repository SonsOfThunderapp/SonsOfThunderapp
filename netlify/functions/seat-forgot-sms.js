  ...
  if (sid && tw && from) {
    try {
      ...
      await fetch('https://api.twilio.com/2010-04-01/Accounts/' + sid + '/Messages.json', {
        ...
        body: 'To=' + encodeURIComponent(phone) + '&From=' + encodeURIComponent(from) + '&Body=' + encodeURIComponent('SONS OF THUNDER. Code: ' + code)
      });
    } catch (e1) {}
  }
  return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ok: true, token: token, exp: exp }) };
};
