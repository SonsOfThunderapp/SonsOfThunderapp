...
## Living Home (do not regress)
21. Gathering-day phases use the canonical meeting engine only (including post: GOOD NIGHT after meeting time on meeting day).
22. Personal greeting: known first name only — never invent identity.
23. Thunder AI action chips: surface only; human executes; no privileged autonomy.

## One breathing unit (do not regress)
19. Ambient life (bolts, brother names, THE CODE, SHARE CONTACT, info labels, locked-in status, FAB bolt, install presence) shares **one** clock: `--tb-breathe` (4s ease-in-out, peak 50%, **animation-delay: 0**). No staggered ambient drift.
20. One-shot effects (I’m In lock-pulse, NEW entrance, fab-hit, tb-press, save reward) stay one-shot — do not convert them into permanent loops that fight the heartbeat.
